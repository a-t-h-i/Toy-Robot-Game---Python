...
def get_robot_name():
    ...
    if is_turtle:
        ninja_turtle.positions = obstacles.my_obstacles
        my_obstacles = ninja_turtle.positions
        
        for i in range(len(ninja_turtle.positions)):
            ninja_turtle.place_obstacle(ninja_turtle.positions[i][0])
    return name
...
def do_forward(robot_name, steps):
    ...
    """
    #At index 0 is the update position boolean
    #At index 1 is the blocked path boolean
    
    
    
    if world.obstacle_check(steps):
...
def robot_start():
    ...
    global position_x, position_y, current_direction_index, history, my_obstacles
